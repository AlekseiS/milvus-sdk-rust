...
async fn high_volume_search_and_query_under_load() -> Result<()> {
    ...
    // 1. Setup
    println!(
        "Phase 2: Setting up collection '{}'...",
        PHASE_2_COLLECTION_NAME
    );
    ...
    println!(
        "Initial population of {} entities complete.",
    ...
    let index_params = IndexParams::new(
        "phase_2_index".to_string(),
        IndexType::IvfFlat,
        MetricType::L2,
        HashMap::new(),
    );
    client
        .create_index(PHASE_2_COLLECTION_NAME, DEFAULT_VEC_FIELD, index_params)
        .await?;
    println!("Index created.");
    client
        .load_collection(PHASE_2_COLLECTION_NAME, None)
        .await?;
    println!(
        "Collection loaded. Starting concurrent tests for {} seconds...",
        TEST_DURATION_SECS
    );
        ...
        tasks.push(tokio::spawn(async move {
            while start_time.elapsed() < test_duration {
                let query_vector: Vec<f32> =
                    (0..DEFAULT_DIM).map(|_| rand::thread_rng().r#gen()).collect();
