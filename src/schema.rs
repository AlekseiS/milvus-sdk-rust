...
pub struct FieldSchema {
    ...
    pub enable_analyzer: bool,    // only for VarChar - enables text analysis for BM25
    pub is_function_output: bool, // true for fields generated by functions (e.g., sparse vectors from BM25)
    pub enable_bm25_stats: bool,  // only for SparseFloatVector - enables BM25 stats collection
}
...
impl From<schema::FieldSchema> for FieldSchema {
    fn from(fld: schema::FieldSchema) -> Self {
        ...
        let enable_analyzer: bool = fld
            .type_params
            .iter()
            .find(|k| &k.key == "enable_analyzer")
            .and_then(|x| x.value.parse().ok())
            .unwrap_or(false);

        let enable_bm25_stats: bool = fld
            .type_params
            .iter()
            .find(|k| &k.key == "enable_bm25_stats")
            .and_then(|x| x.value.parse().ok())
            .unwrap_or(false);
        ...
        FieldSchema {
            ...
            dim,
            enable_analyzer,
            is_function_output: fld.is_function_output,
            enable_bm25_stats,
        }
    }
}
...
impl FieldSchema {
    ...
    /// Enables text analyzer for this VARCHAR field (used for BM25 full-text search).
    /// Only applicable for VARCHAR fields.
    pub fn with_analyzer(mut self) -> Self {
        self.enable_analyzer = true;
        self
    }

    /// Creates a sparse float vector field.
    /// Note: Sparse float vectors can be user-provided or generated by functions like BM25.
    /// Use `mark_as_function_output()` if this field will be populated by a function.
    ...
    /// Marks this field as a function output field (e.g., sparse vectors generated by BM25).
    /// Function output fields are automatically populated and should not be provided during insert.
    pub fn mark_as_function_output(mut self) -> Self {
        self.is_function_output = true;
        self
    }

    /// Enables BM25 statistics collection for sparse float vector fields.
    /// This is required when using BM25 metric type for search.
    pub fn with_bm25_stats(mut self) -> Self {
        self.enable_bm25_stats = true;
        self
    }
    ...
    fn from(fld: FieldSchema) -> schema::FieldSchema {
        let params = match fld.dtype {
            ...
            DataType::SparseFloatVector => {
                let mut p = Vec::new();
                if fld.enable_bm25_stats {
                    p.push(KeyValuePair {
                        key: "enable_bm25_stats".to_string(),
                        value: "true".to_string(),
                    });
                }
                p
